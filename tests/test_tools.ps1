...
Foreach (${TestName} in ${ToolsTests} -split " ")
{
	...
	$ResultRun = RunTestBinary ${TestExecutablesDirectory} "vsbsdl_test_tools_${TestName}"
...
For ($ProfileIndex = 0; $ProfileIndex -le ($Profiles.length - 1); $ProfileIndex += 1)
{
	...
	{
		$ResultRun = RunToolsBinaryAndCompareStdout ${TestExecutablesDirectory} "vsbsdlinfo" ${TestProfile} "" ${TestInput}
